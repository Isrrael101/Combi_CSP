...
def IAM_tro3(hoy:np.array=HOYS_DEFAULT):
    ...
    #return cos(radians(thetai(hoy))) + 0.000884 * thetai(hoy) - 0.00005369 * (thetai(hoy))**2 # needs rad despite thetai(hoy) already in rad???
...
def IAM_tro4(hoy,foc_len,area,L):
    ...
    return 1 - foc_len / L *(1 + area**2 / 48 * foc_len**2) * np.tan(np.radians(thetai(hoy))) # needs rad despite thetai(hoy) already in rad???
...
def CSCP(Tfi, hoy:np.array= HOYS_DEFAULT, fname:str="tmy_35.015_25.755_2005_2020.csv"): 
